...
GEMSPEC_PATH = File.expand_path("../rfc-reader.gemspec", __dir__).freeze
...
if gemspec_min_ruby_version.nil?
  warn "Unable to parse required_ruby_version from rfc-reader.gemspec"
  exit(1)
end
...
if standard_rb_ruby_version != gemspec_min_ruby_version
  warn <<~ERROR
    .standard.yml ruby version is #{standard_rb_ruby_version} and
      rfc-reader.gemspec required_ruby_version is #{gemspec_min_ruby_version}
      which do not match.
  ERROR
  exit(1)
end
